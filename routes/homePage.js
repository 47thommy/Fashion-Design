...
router.get("/tag", async (req, res) => {
  try {
    ...
    if (!req.query) throw "need tagInfo";
    if (!req.query.searchTag) throw "need a tag";
...
router.get("/search", async (req, res) => {
  try {
    // search?searchString=xxxx
    let userLogin = null;
    if (req.session) {
      if (req.session.userId)
        userLogin = await userData.getUserById(req.session.userId);
    }
    if (!req.query) throw "need string to search";
    if (!req.query.searchString) throw "need string to search!";
...
router.post("/createPost", async (req, res) => {
  ...
  form.parse(req, async (err, fields, files) => {
    try {
      if (!fields) throw "need data to create post";
      if (!fields.topic)
        //标题
      ...
      if (!Array.isArray(tagArr)) throw "need a tagArr to create post";
      let photoArr = [];
