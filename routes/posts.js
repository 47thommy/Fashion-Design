...
router.get("/postInfo/:id", async (req, res) => {
  try {
    ...
    for (let i = 0; i < postInfo.commentIdArr.length; i++) {
      let thisComment = await commentData.getCommentById(
        postInfo.commentIdArr[i]
      );
...
router.get("/like", async (req, res) => {
  try {
    ...
    if (!req.session) throw "you don't have the cookie";
    if (!req.session.userId) throw "login first,then like it";
    if (!req.query) throw "need info to like it";
    if (!req.query.postId) throw "need postId to like it";
    let updatedPost = await postData.addLikeCount(
      req.query.postId,
      req.session.userId
    );
...
router.get("/dislike", async (req, res) => {
  try {
    if (!req.session) throw "you don't have the cookie";
    if (!req.session.userId) throw "login first,then dislike it";
    if (!req.query) throw "need info to dislike it";
    if (!req.query.postId) throw "need postId to dislike it";
    let updatedPost = await postData.addDislikeCount(
      req.query.postId,
      req.session.userId
    );
    res.send(updatedPost);
...
router.post("/editContent", async (req, res) => {
  try {
    if (!req.session) throw "you don't have the cookie";
    if (!req.session.userId) throw "login first,then edit content";
    if (!req.body) throw "need new content and postId";
    if (!req.body.postId) throw "need postId";
    if (!req.body.newContent) throw "new Content";
    let postToEdit = await postData.getPostById(req.body.postId);
    if (postToEdit.userId !== req.session.userId)
      //这里判断想要修改post content的人是不是真正的写这个post的人
      throw "Your id is not the same as the userId of the post!!";
    let updatedPost = await postData.editContent(
      req.body.postId,
      req.body.newContent
    );
    res.redirect("http://44.226.145.213 /users/account");
  } catch (error) {
    res.redirect("http://44.226.145.213 /users/account");
  }
});
...
router.post("/delete", async (req, res) => {
  try {
    ...
    if (!req.session.userId)
      throw "you dont have userId in session to delete the post";
...
router.post("/addComment", async (req, res) => {
  try {
    if (!req.session) throw "you don't have the cookie";
    if (!req.session.userId) throw "login first,then make commnet";
    if (!req.body) throw "need info to create a comment";
    if (!req.body.postId) throw "need postId to create a comment";
    if (!req.body.commentContent)
      throw "need commentContent to create a comment";
    let newComment = await commentData.addComment(
      req.body.postId,
      req.session.userId,
      req.body.commentContent
    );
    res.redirect("http://44.226.145.213 /posts/postInfo/" + req.body.postId);
  } catch (error) {
    res.redirect("http://44.226.145.213 /posts/postInfo/" + req.body.postId);
  }
});
...
router.post("/deleteComment", async (req, res) => {
  try {
    if (!req.session) throw "you don't have the cookie to delete the comment";
    if (!req.session.userId) throw "login first,then delete commnet";
    if (!req.body) throw "need info to delete the comment";
    if (!req.body.postId) throw "need postId to delete the comment";
...
router.post("/removeReport", async (req, res) => {
  ...
  try {
    ...
    if (!req.session) throw "you dont have session to delete the post";
    if (!req.session.userId)
      throw "you dont have userId in session to delete the report";
    if (!req.body.reportId)
      throw "you dont have reportId in body to delete the report";
    let deletePerson = await userData.getUserById(req.session.userId);
    let postDelete = null;
    if (deletePerson.admin === false) res.send("no access!!!");
    else {
